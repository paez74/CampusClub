    ...
    <entity-list-table :config="tableConfig" :list="list" :loading="loading">
      <template slot-scope="{ props }">
        ...
        <td class="text-xs-left">
          <v-icon :color="props.item.active ? 'accent' : 'secondary'">{{
            props.item.active ? 'done' : 'close'
          }}</v-icon>
        </td>
      </template>
    </entity-list-table>
  ...
  data() {
    return {
      ...
      tableConfig: {
        ...
        actions: (type, item) => {
          const actionsMap = {
            view: () => this.goTo('roledetail/' + item.id),
            edit: () => this.goTo('roleform/' + item.id),
