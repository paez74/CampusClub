<template>
	<div>
				...
				<v-card-text>
					...
					<v-expansion-panel>
						<v-expansion-panel-content
							v-for="category in icons_filter.categories"
							:key="category.key"
						>
							...
							<v-card>
								<v-container grid-list-md>
									<v-layout row wrap>
										<v-flex xs2 v-for="icon in category.icons" :key="icon.id">
											<v-btn
												fab
												dark
												small
												color="primary"
												@click="select(icon.ligature)"
											>
												<v-icon>{{ icon.ligature }}</v-icon>
											</v-btn>
										</v-flex>
									</v-layout>
								</v-container>
		...
		<div v-if="disabled" class="display-icon">
			<div class="icon-wrapper primary">
				<v-icon dark>{{ value }}</v-icon>
			</div>
...
.display-icon > .icon-wrapper > .material-icons {
	font-size: 18px;
}
...
export default {
	...
	methods: {
		...
		filter() {
			...
			this.timeout = setTimeout(function() {
				self.timeout = null;
				var result = self.filterData(self.icons.categories, function(item) {
					return item.name.includes(self.search);
				});
				self.icons_filter.categories = result;
			}, 500);
		},
		filterData(data, predicate) {
			return !!!data
				...
				: data.reduce((list, entry) => {
						...
						} else if (entry.icons != null) {
							let icons = this.filterData(entry.icons, predicate);
							if (icons.length > 0) {
								clone = Object.assign({}, entry, { icons: icons });
							}
						}
	...
	mounted() {
		this.icons_filter.categories = this.icons.categories;
	},
	computed: {
		internalValue() {
			return this.value;
